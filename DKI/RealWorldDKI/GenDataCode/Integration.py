...
def generate_data_main():
    ...
    parser.add_argument("--output_file", type=str, default="DKI/RealWorldDKI/InterventionDataset/RealWorld_Integration.json")
    ...
    instruction = (
        ...
        "- As you read from top to bottom, maintain for each cue in CUE an evolving chain: CUE: v(1) → v(2) → … → v(u), where v(1) is the value from the first occurrence and v(u) is the value from the last occurrence.\n"
        "- Do NOT treat records as independent pairs. Treat each later occurrence of the same cue as an update to the same cue, so that the earliest value is v(1) and the latest value is v(u)"
        "\n"
    )
    input_text = (
